...
- [x] M7: default-elastic geometry — on-demand allocation is now the DEFAULT: a create that names no geometry starts small (256 MiB / 1 vCPU) and autoscales up to a configurable ceiling (4 GiB / 4 vCPU) instead of being fixed at creation; elastic sandboxes fast-create from a second per-template ELASTIC golden snapshot (hotplug region baked in) instead of cold-booting; runtime autoscale toggle, resize/autoscale/migrate timeline events, node oversell reporting, and a full console resource-management surface ([ADR-0008](docs/adr/0008-default-elastic-geometry.md); exit criteria — elastic fast-create P50 <500ms, hotplug resize on a jailed golden clone across pause/resume, memmap-tax probe — gated on nested-virt CI)
